...
var chart_data: Array[Dictionary] = [
	...
	{type = &'catch', section = 1},
	{type = &'catch', section = 1},
	{type = &'tap', section = 1, hold_points = [
		[1.5, 0],
	]},
]
...
static func get_vertical_note_field_position(normal: float) -> float:
...
func _ready() -> void:
	...
	for note_data in chart_data:
		...
		if note_data.get(&'type') == &'tap':
			...
			if note_data.has(&'hold_points'):
				for hold_point_data in note_data.get(&'hold_points'):
					var hold_point: HoldPoint = preload("res://hold_point.tscn").instantiate()
					hold_point.time = hold_point_data[0]
					hold_point.placement = hold_point_data[1]
					note.hold_points.append(hold_point)
